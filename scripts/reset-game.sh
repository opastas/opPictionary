...
if ! curl -s http://192.168.1.13:4000/health > /dev/null; then
    echo "❌ Server is not running. Please start the server first."
    exit 1
fi

# Reset the game state
RESPONSE=$(curl -s -X POST http://192.168.1.13:4000/reset)
...
echo "🌐 Web App: http://192.168.1.13:5173"
echo "📱 Mobile App: http://192.168.1.13:8081"
